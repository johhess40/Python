...
def marvelcharcall(stampystamp, hashyhash, pkeyz, lookmeup):
    r = requests.get(XAVIER + "?name=" + lookmeup + "&ts=" +
                     stampystamp + "&apikey=" + pkeyz + "&hash=" + hashyhash)

    print(XAVIER + "?name=" + lookmeup + "&ts=" + stampystamp +
          "&apikey=" + pkeyz + "&hash=" + hashyhash)
    return r.json()
...
def main():
    ...
    # build hash with hashbuilder(timestamp, privatekey, publicekey)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pub',
                        help='Provide the /path/to/file.pub containg Marvel public developer key')

    parser.add_argument('--hero', \
    help='Character to search for within the Marvel ')
